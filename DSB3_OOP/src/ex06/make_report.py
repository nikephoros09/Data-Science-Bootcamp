...
if __name__ == '__main__':
    try:
        ...
        research.tg_message(False, API_TOKEN)
    else:
        final_res = REP_TEMPLATE.format(
            flips=flips,
            tails=tails,
            heads=heads,
            tails_percent=tails_fraction * 100,
            heads_percent=heads_fraction * 100,
            predict_flips=predict_flips,
            predict_tails=predict_tails,
            predict_heads=predict_heads
        )
        print(final_res)
        Research.Analytics.save_file(final_res, "test", "txt")
        research.tg_message(True, API_TOKEN)